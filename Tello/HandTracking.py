...
def findCommand(img):
    ...
    if len(lmList) != 0:
        ...
        command_conditions = {
            1: lambda lmList, distance: fingerDown(lmList, 8, 7) and fingerDown(lmList, 12, 11) and fingerDown(lmList, 16, 15) and (250 > distance > 200),
            2: lambda lmList, distance2: fingerDown(lmList, 20, 19) and fingerDown(lmList, 12, 11) and fingerDown(lmList, 16, 15) and (300 > distance2 > 180),
        }

        for command, condition_func in command_conditions.items():
            if condition_func(lmList, distance) or condition_func(lmList, distance2):
                return command
    return 0
...
def doCommand(tello, command):
    ...
    command_functions.get(command, lambda: None)()
